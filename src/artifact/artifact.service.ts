...
import { RESTRICTION, TARGET_RESTRICTION, TargetRestriction } from 'src/action/types/restriction';
...
import { ARTIFACT, Artifact, ArtifactNeighbors, SPAWN_POSITION, SpawnPosition } from './types/artifact';
...
export class ArtifactService {
    ...
    getFaceAction(artifact: ArtifactGameState, player: Player, enemy: Player) {
        ...
        const possibleTargets = this.restrictionService.getTargetsByRestrictions(player, enemy, targetRestrictions);
    ...
    getSkills(player: Player, enemy: Player, artifact: ArtifactGameState): SkillStateType[] {
        ...
        if (this.gameEffectsService.countEffect(artifact, EFFECT.ARTIFACT_SILENCE) > 0) {
            return [];
        }
        ...
        ARTIFACTS[artifact.artifactId].skills?.forEach((skill) => {
            ...
            if (player.resources[RESOURCE.RAGE] >= cost) {
                if (this.restrictionService.checkGeneralRestrictions(player, enemy, skillData.restrictions)) {
                    if (this.restrictionService.checkArtifactRestrictions(skillData.restrictions, player, artifact)) {
    ...
    spawnArtifact(
        ...
        if (position === SPAWN_POSITION.FRONT_LINE) {
            positionInsert = Object.values(artifacts).filter(a => a.line === LINE.FRONT).length;
            line = LINE.FRONT
        } 
            ...
            if (spawnerPosition === 0) {
                ...
                positionInsert = random === 0 ? spawnerPosition - 1 : spawnerPosition + 1;
            }
    ...
    createArtifactState(playerArtifacts: Record<string, ArtifactGameState>, artifactId: Artifact) {
        ...
        if (ARTIFACTS[artifactId].skills && ARTIFACTS[artifactId].skills!.length > 0) {
    ...
    destroyArtifact(player: Player, artifact: ArtifactGameState, logParts: string[]) {
        const countArtifactOnSameLine = Object.values(player.artifacts).filter(a => a.line === artifact.line).length;
