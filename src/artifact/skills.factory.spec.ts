...
import { CommonException, COMMON_ERROR_CODE } from '../common/utils/error-handler';

// Mock strategies
class MockFearStrategy implements SkillStrategy {
  getSkillType(): Skill { return SKILL.FEAR; }
  execute = jest.fn();
  death = jest.fn();
}

class MockFrozeStrategy implements SkillStrategy {
  getSkillType(): Skill { return SKILL.FROZE; }
  execute = jest.fn();
  death = jest.fn();
}

class MockSwiftStrategy implements SkillStrategy {
  getSkillType(): Skill { return SKILL.SWIFT; }
  execute = jest.fn();
  death = jest.fn();
}

class MockNeighboringHealingStrategy implements SkillStrategy {
  getSkillType(): Skill { return SKILL.NEIGHBORING_HEALING; }
  execute = jest.fn();
  death = jest.fn();
}
...
describe('SkillsStrategyFactory', () => {
  ...
  describe('getStrategy', () => {
    ...
    it('should throw CommonException for unknown skill type', () => {
      expect(() => factory.getStrategy('unknown_skill' as Skill)).toThrow(CommonException);
  ...
  describe('factory initialization', () => {
    it('should build handlers map on construction', () => {
      ...
      expect(strategies.get(SKILL.NEIGHBORING_HEALING)).toBe(mockStrategies[3]);
  ...
  describe('edge cases', () => {
    it('should handle empty strategies list', async () => {
      ...
      const emptyFactory = module.get<SkillsStrategyFactory>(SkillsStrategyFactory);
      
      expect(() => emptyFactory.getStrategy(SKILL.FEAR)).toThrow(CommonException);
    });
    ...
    it('should handle duplicate strategies (last one wins)', async () => {
      ...
      const duplicateFactory = module.get<SkillsStrategyFactory>(SkillsStrategyFactory);
