...
describe('SpawnBonesStrategy', () => {
  ...
  const createMockArtifact = (id: string = 'artifact-1', position: number = 1, line: Line = LINE.FRONT): ArtifactGameState => ({
    id,
    artifactId: 'bonelord',
    face: 'sword',
  ...
  describe('execute', () => {
    ...
    beforeEach(() => {
      ...
      mockArtifactService.createArtifactState.mockReturnValue(mockBoneKnife);
    ...
    it('should create and spawn two Bone Knife artifacts', () => {
      ...
      expect(mockArtifactService.createArtifactState).toHaveBeenCalledTimes(2);
      expect(mockArtifactService.createArtifactState).toHaveBeenCalledWith(
        player.artifacts,
        ARTIFACT.BONE_KNIFE
      );
    });
