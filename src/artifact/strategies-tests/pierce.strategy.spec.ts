...
jest.mock('../../game-mechanics/constants/effects', () => ({
  EFFECTS: {
    'pierce': { id: 'pierce', name: 'Pierce' },
  },
}));

describe('PierceStrategy', () => {
  let strategy: PierceStrategy;
  ...
  beforeEach(async () => {
    ...
    strategy = module.get<PierceStrategy>(PierceStrategy);
  ...
  describe('getSkillType', () => {
    it('should return PIERCE', () => {
      expect(strategy.getSkillType()).toBe(SKILL.PIERCE);
    });
  });
  ...
  describe('execute', () => {
    ...
    it('should apply PIERCE effect if not already present and not exhausted', () => {
    ...
    it('should not apply PIERCE effect if already present', () => {
      gameEffectsService.countEffect.mockImplementation((artifact, effect) => {
        if (effect === EFFECT.PIERCE) return 1;
        return 0;
      });
    ...
    it('should not apply PIERCE effect if exhausted', () => {
      gameEffectsService.countEffect.mockImplementation((artifact, effect) => {
