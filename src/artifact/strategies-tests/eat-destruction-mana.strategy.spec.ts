...
describe('EatDestructionManaStrategy', () => {
  let strategy: EatDestructionManaStrategy;
  ...
  beforeEach(async () => {
    ...
    strategy = module.get<EatDestructionManaStrategy>(EatDestructionManaStrategy);
  ...
  describe('getSkillType', () => {
    it('should return EAT_DESTRUCTION_MANA', () => {
      expect(strategy.getSkillType()).toBe(SKILL.EAT_DESTRUCTION_MANA);
    });
  });
  ...
  describe('execute', () => {
    ...
    beforeEach(() => {
      ...
      data = {
        skillId: SKILL.EAT_DESTRUCTION_MANA,
        gameId: 'game-123',
    ...
    it('should decrease destruction mana and heal artifact', () => {
      ...
      expect(combatService.calculateHeal).toHaveBeenCalledWith(artifact, 15);
      expect(combatService.applyHealing).toHaveBeenCalledWith(artifact, 15, logParts);
    });
