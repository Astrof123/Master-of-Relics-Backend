...
jest.mock('../../game-mechanics/constants/effects', () => ({
  EFFECTS: {
    'light_mana_discount': { id: 'light_mana_discount', name: 'Light Mana Discount' },
  },
}));

describe('LightManaDiscountStrategy', () => {
  let strategy: LightManaDiscountStrategy;
  ...
  beforeEach(async () => {
    ...
    strategy = module.get<LightManaDiscountStrategy>(LightManaDiscountStrategy);
  ...
  describe('getSkillType', () => {
    it('should return LIGHT_MANA_DISCOUNT', () => {
      expect(strategy.getSkillType()).toBe(SKILL.LIGHT_MANA_DISCOUNT);
    });
  });
  ...
  describe('execute', () => {
    ...
    beforeEach(() => {
      ...
      data = {
        skillId: SKILL.LIGHT_MANA_DISCOUNT,
        gameId: 'game-123',
    ...
    it('should apply light mana discount effect if not exhausted and not already applied', () => {
    ...
    it('should not apply duplicate effect from same artifact', () => {
      const existingEffect = { id: EFFECT.LIGHT_MANA_DISCOUNT, effectCasterGameId: 'artifact-1' };
  ...
  describe('death', () => {
    ...
    it('should remove light mana discount effect on death', () => {
      const { EFFECTS } = require('../../game-mechanics/constants/effects');
