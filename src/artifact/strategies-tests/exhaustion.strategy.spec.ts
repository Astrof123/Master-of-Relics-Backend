...
jest.mock('../../game-mechanics/constants/effects', () => ({
  EFFECTS: {
    'exhaustion': { id: 'exhaustion', name: 'Exhaustion', duration: 'always', type: 'negative', number: null, dispellType: 'normal' },
  },
}));
...
describe('ExhaustionStrategy', () => {
  ...
  const createMockEffect = (id: string, dispellType: string, effectCasterGameId?: string): EffectType => ({
    id,
    name: 'Test Effect',
    duration: 'always',
    type: 'negative',
  ...
  const createMockArtifact = (id: string = 'enemy-artifact', effects: EffectType[] = []): ArtifactGameState => ({
    id,
  ...
  describe('execute', () => {
    ...
    it('should remove PASSIVE effects from enemy artifact', () => {
      ...
      gameState.enemy.artifacts['enemy-artifact'].effects = [passiveEffect];
    ...
    it('should remove PASSIVE hero effects from enemy with matching caster', () => {
      const heroEffect = createMockEffect('hero1', 'passive', 'enemy-artifact');
