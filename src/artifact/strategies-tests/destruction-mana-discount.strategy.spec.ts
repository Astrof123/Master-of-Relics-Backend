// strategies-tests/destruction-mana-discount.strategy.spec.ts
...
jest.mock('../../game-mechanics/constants/effects', () => ({
  EFFECTS: {
    'destruction_mana_discount': { id: 'destruction_mana_discount', name: 'Destruction Mana Discount' },
  },
}));

describe('DestructionManaDiscountStrategy', () => {
  let strategy: DestructionManaDiscountStrategy;
  ...
  const createMockArtifact = (id: string = 'artifact-1'): ArtifactGameState => ({
    id,
    artifactId: 'ring_of_destruction',
    face: 'sword',
  ...
  beforeEach(async () => {
    ...
    strategy = module.get<DestructionManaDiscountStrategy>(DestructionManaDiscountStrategy);
  ...
  describe('getSkillType', () => {
    it('should return DESTRUCTION_MANA_DISCOUNT', () => {
      expect(strategy.getSkillType()).toBe(SKILL.DESTRUCTION_MANA_DISCOUNT);
    });
  });
  ...
  describe('execute', () => {
    ...
    beforeEach(() => {
      ...
      data = {
        skillId: SKILL.DESTRUCTION_MANA_DISCOUNT,
        gameId: 'game-123',
    ...
    it('should apply destruction mana discount effect if not exhausted and not already applied', () => {
  ...
  describe('death', () => {
    ...
    it('should remove destruction mana discount effect on death', () => {
      const { EFFECTS } = require('../../game-mechanics/constants/effects');
      ...
      expect(gameEffectsService.removeHeroEffect).toHaveBeenCalledWith(
        player,
        EFFECTS['destruction_mana_discount'],
        logParts
      );
    });
  });
