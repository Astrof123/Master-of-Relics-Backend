...
@Injectable()
export class LightManaDiscountStrategy implements SkillStrategy {
    constructor(
    ...
    getSkillType(): Skill {
        return SKILL.LIGHT_MANA_DISCOUNT;
    }
    ...
    execute(gameState: GameForLogic, player: Player, artifact: ArtifactGameState, data: UseSkillData, animations: AnimationData[], logParts: string[]) {
        if (this.gameEffectsService.countEffect(artifact, EFFECT.EXHAUSTION) === 0) {
            const effectsLightDiscount = this.gameEffectsService.getHeroEffects(player, EFFECT.LIGHT_MANA_DISCOUNT);
            let hasEffectFromThisArtifact = false;
            
            for (const effect of effectsLightDiscount) {
            ...
            if (!hasEffectFromThisArtifact) {
                const effect: EffectType = EFFECTS[EFFECT.LIGHT_MANA_DISCOUNT];
    ...
    death(gameState: GameForLogic, player: Player, artifact: ArtifactGameState, logParts: string[]) {
        this.gameEffectsService.removeHeroEffect(
            player,
            EFFECTS[EFFECT.LIGHT_MANA_DISCOUNT], 
            logParts
        )
    }
}