...
@Injectable()
export class StealDestructionManaStrategy implements SkillStrategy {
    constructor(
    ...
    getSkillType(): Skill {
        return SKILL.STEAL_DESTRUCTION_MANA;
    }
    ...
    execute(gameState: GameForLogic, player: Player, artifact: ArtifactGameState, data: UseSkillData, animations: AnimationData[], logParts: string[]) {
        ...
        this.resourceService.decreaseResource(enemy, RESOURCE.DESTRUCTION_MANA, 10, logParts);
        this.resourceService.addResource(player, RESOURCE.DESTRUCTION_MANA, 10, logParts);
    }
