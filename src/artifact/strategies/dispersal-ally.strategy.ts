...
@Injectable()
export class DispersalAllyStrategy implements SkillStrategy {
    constructor(
    ...
    execute(gameState: GameForLogic, player: Player, artifact: ArtifactGameState, data: UseSkillData, animations: AnimationData[], logParts: string[]) {
        ...
        for (const effect of allyArtifact.effects) {
            if (effect.dispellType === DISPELL_TYPE.NORMAL && effect.type === "negative") {
