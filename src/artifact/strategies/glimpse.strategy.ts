...
@Injectable()
export class GlimpseStrategy implements SkillStrategy {
    constructor(
    ...
    execute(gameState: GameForLogic, player: Player, artifact: ArtifactGameState, data: UseSkillData, animations: AnimationData[], logParts: string[]) {
        if (this.gameEffectsService.countEffect(artifact, EFFECT.GLIMPSE) === 0 && this.gameEffectsService.countEffect(artifact, EFFECT.EXHAUSTION) === 0) {
            this.gameEffectsService.applyEffect(
                artifact,
                EFFECTS[EFFECT.GLIMPSE], 
                logParts
            )
        }
    }
