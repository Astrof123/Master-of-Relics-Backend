...
jest.mock('../action/helpers/logHelper', () => ({
    LogHelper: {
        getAppliedStateLog: jest.fn(
            (artifactName, state) =>
                `${artifactName} перешёл в состояние ${state}`,
        ),
    },
}));
...
describe('ArtifactStateService', () => {
    ...
    describe('applyState', () => {
        ...
        it('should work with different artifact types', () => {
            const moonStaffArtifact = createMockArtifact(
                'artifact-2',
                'moon_staff',
                ARTIFACT_STATE.READY_TO_USE,
            );
    ...
    describe('clearDestroyedArtifacts', () => {
        ...
        it('should remove destroyed artifacts from player', () => {
            const destroyedArtifact = createMockArtifact(
                'destroyed-1',
                'arcane_shield',
                ARTIFACT_STATE.DESTROYED,
            );
            const normalArtifact = createMockArtifact(
                'normal-1',
                'moon_staff',
                ARTIFACT_STATE.READY_TO_USE,
            );
    ...
    describe('updateStateNewRound', () => {
        ...
        it('should skip BREAKEN artifacts', () => {
            const brokenArtifact = createMockArtifact(
                'broken-1',
                'arcane_shield',
        ...
        it('should handle both player and enemy artifacts', () => {
            const playerArtifact = createMockArtifact(
                'player-artifact',
                'arcane_shield',
                ARTIFACT_STATE.COOLDOWN,
            );
            const enemyArtifact = createMockArtifact(
                'enemy-artifact',
                'moon_staff',
                ARTIFACT_STATE.STUNNED,
            );
        ...
        it('should preserve artifacts already in READY_TO_USE state', () => {
            const readyArtifact = createMockArtifact(
                'ready-1',
                'arcane_shield',
                ARTIFACT_STATE.READY_TO_USE,
            );
