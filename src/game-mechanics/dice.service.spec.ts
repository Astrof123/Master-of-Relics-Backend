...
describe('DiceService', () => {
    ...
    describe('throwDice', () => {
        ...
        it('should throw dice and update artifact face', () => {
            ...
            const result = service.throwDice(
                player,
                artifactGameId,
                artifactId,
                logParts,
        ...
        it('should handle different dice results', () => {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                const expectedFace = require('../artifact/constants/artifacts')
                    .ARTIFACTS[artifactId].faces[i];
                expect(newPlayer.artifacts[artifactGameId].face).toBe(
                    expectedFace,
                );
            }
        });
        ...
        it('should work with different artifact types', () => {
            ...
            for (const testCase of testCases) {
                ...
                service.throwDice(
                    newPlayer,
                    testCase.artifactGameId,
                    testCase.artifactId,
                    [],
                );

                const expectedFace = require('../artifact/constants/artifacts')
                    .ARTIFACTS[testCase.artifactId].faces[0];
                expect(newPlayer.artifacts[testCase.artifactGameId].face).toBe(
                    expectedFace,
                );
            }
        });
    ...
    describe('updateDicesNewRound', () => {
        ...
        it('should throw dice for each artifact', () => {
            const playerArtifactCount = Object.keys(
                gameState.player.artifacts,
            ).length;
            const enemyArtifactCount = Object.keys(
        ...
        it('should handle player with no artifacts but enemy has', () => {
            gameState.player.artifacts = {};
            gameState.enemy.artifacts = {
                'enemy-artifact-1': createMockArtifact(
                    'enemy-artifact-1',
                    'axe_of_the_berserker',
                ),
            };
    ...
    describe('Edge cases', () => {
        ...
        it('should handle multiple dice throws in sequence', () => {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                const expectedFace = require('../artifact/constants/artifacts')
                    .ARTIFACTS[artifactId].faces[results[i]];
                expect(newPlayer.artifacts[artifactGameId].face).toBe(
                    expectedFace,
                );
            }
    ...
    describe('Integration with game state', () => {
        it('should maintain other artifact properties after dice throw', () => {
            ...
            expect(player.artifacts[artifactGameId].id).toBe(
                originalArtifact.id,
            );
            expect(player.artifacts[artifactGameId].artifactId).toBe(
                originalArtifact.artifactId,
            );
            expect(player.artifacts[artifactGameId].currentHp).toBe(
                originalArtifact.currentHp,
            );
            expect(player.artifacts[artifactGameId].maxHp).toBe(
                originalArtifact.maxHp,
            );
            expect(player.artifacts[artifactGameId].state).toBe(
                originalArtifact.state,
            );
        });
