...
describe('ResourceService', () => {
  ...
  describe('addResource', () => {
    ...
    it('should add destruction mana resource correctly', () => {
      service.addResource(player, RESOURCE.DESTRUCTION_MANA, 30, logParts);
    ...
    it('should not exceed MAX_AMOUNT_RESOURCES', () => {
      ...
      expect(player.resources[RESOURCE.AGILITY]).toBe(MAX_AMOUNT_RESOURCES);
    });
  ...
  describe('decreaseResource', () => {
    ...
    it('should decrease destruction mana resource correctly', () => {
      service.decreaseResource(player, RESOURCE.DESTRUCTION_MANA, 3, logParts);
  ...
  describe('addResourceNewRound', () => {
    it('should add 15 agility and 15 rage to both players', () => {
      ...
      const initialPlayerAgility = gameState.player.resources[RESOURCE.AGILITY];
      const initialPlayerRage = gameState.player.resources[RESOURCE.RAGE];
      const initialEnemyAgility = gameState.enemy.resources[RESOURCE.AGILITY];
      const initialEnemyRage = gameState.enemy.resources[RESOURCE.RAGE];

      service.addResourceNewRound(gameState);

      expect(gameState.player.resources[RESOURCE.AGILITY]).toBe(initialPlayerAgility + 15);
      expect(gameState.player.resources[RESOURCE.RAGE]).toBe(initialPlayerRage + 15);
      expect(gameState.enemy.resources[RESOURCE.AGILITY]).toBe(initialEnemyAgility + 15);
      expect(gameState.enemy.resources[RESOURCE.RAGE]).toBe(initialEnemyRage + 15);
    });
    ...
    it('should respect MAX_AMOUNT_RESOURCES when adding resources', () => {
      ...
      expect(gameState.player.resources[RESOURCE.AGILITY]).toBe(MAX_AMOUNT_RESOURCES);
      expect(gameState.player.resources[RESOURCE.RAGE]).toBe(MAX_AMOUNT_RESOURCES);
      expect(gameState.enemy.resources[RESOURCE.AGILITY]).toBe(MAX_AMOUNT_RESOURCES);
      expect(gameState.enemy.resources[RESOURCE.RAGE]).toBe(MAX_AMOUNT_RESOURCES);
    });
  });
  ...
  describe('calculateNewTurnMovePoints', () => {
    ...
    it('should return 2 when player has one EXTRA_MOVE effect', () => {
      player.effects = [
        { id: EFFECT.EXTRA_MOVE, name: 'Extra Move', duration: 'current_round', type: 'positive', number: null, dispellType: 'normal' },
      ];
    ...
    it('should ignore other effects', () => {
      player.effects = [
        { id: EFFECT.EXTRA_MOVE, name: 'Extra Move', duration: 'current_round', type: 'positive', number: null, dispellType: 'normal' },
        { id: EFFECT.BERSERK, name: 'Berserk', duration: 'always', type: 'positive', number: null, dispellType: 'passive' },
        { id: EFFECT.HUNT, name: 'Hunt', duration: 'always', type: 'positive', number: null, dispellType: 'passive' },
      ];
  ...
  describe('Multiple operations', () => {
    ...
    it('should handle mixed add and decrease operations', () => {
      ...
      expect(player.resources[RESOURCE.AGILITY]).toBe(50 + 30 - 20 + 10 - 5);
    });
  });
});