...
export class ArtifactStateService {
    ...
    applyState(
        ...
        const currentState = artifactState.state;
        if (
            (state === ARTIFACT_STATE.ROOTED ||
                state === ARTIFACT_STATE.DREAM) &&
            (currentState === ARTIFACT_STATE.READY_TO_USE ||
                currentState === ARTIFACT_STATE.COOLDOWN)
        ) {
