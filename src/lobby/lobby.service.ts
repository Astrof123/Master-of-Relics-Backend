...
export class LobbyService {
    ...
    async createLobby(
        ...
        if (
            !lobbyData.name ||
            lobbyData.name.length < 3 ||
            lobbyData.name.length > 20
        ) {
        ...
        let timerDraft =
            lobbyData.timerDraft < MIN_TIMER_VALUE
                ? MIN_TIMER_VALUE
                : lobbyData.timerDraft;
        timerDraft =
            timerDraft > MAX_TIMER_VALUE ? MAX_TIMER_VALUE : timerDraft;

        let timerTurn =
            lobbyData.timerTurn < MIN_TIMER_VALUE
                ? MIN_TIMER_VALUE
                : lobbyData.timerTurn;
        timerTurn = timerTurn > MAX_TIMER_VALUE ? MAX_TIMER_VALUE : timerTurn;

        let timerMovement =
            lobbyData.timerMovement < MIN_TIMER_VALUE
                ? MIN_TIMER_VALUE
                : lobbyData.timerMovement;
        timerMovement =
            timerMovement > MAX_TIMER_VALUE ? MAX_TIMER_VALUE : timerMovement;

        const code = uid(6).toUpperCase();

        await this.redisService.setJson<Lobby>(
            key,
            '.',
            {
                id: lobbyId,
    ...
    async updateOptionsLobby(
        lobbyOptions: UpdateOptionsLobbyData,
        userId: string,
    ): Promise<string> {
        ...
        let timerDraft =
            lobbyOptions.timerDraft < MIN_TIMER_VALUE
                ? MIN_TIMER_VALUE
                : lobbyOptions.timerDraft;
        timerDraft =
            timerDraft > MAX_TIMER_VALUE ? MAX_TIMER_VALUE : timerDraft;

        let timerTurn =
            lobbyOptions.timerTurn < MIN_TIMER_VALUE
                ? MIN_TIMER_VALUE
                : lobbyOptions.timerTurn;
        timerTurn = timerTurn > MAX_TIMER_VALUE ? MAX_TIMER_VALUE : timerTurn;

        let timerMovement =
            lobbyOptions.timerMovement < MIN_TIMER_VALUE
                ? MIN_TIMER_VALUE
                : lobbyOptions.timerMovement;
        timerMovement =
            timerMovement > MAX_TIMER_VALUE ? MAX_TIMER_VALUE : timerMovement;
        ...
        await this.redisService.setJson<Lobby>(
            ...
            '.',
            {
                ...
                options: {
                    ...
                    timerMovement: lobbyOptions.withTimers
                        ? timerMovement
                        : null,
                    mode: 'classic',
                },
    ...
    async joinLobby(lobbyId: string, userId: string): Promise<void> {
        ...
        await this.redisService.setJson(
            key,
            LOBBYPATH.getPlayerPath(userId),
            newPlayer,
        );
    }
    ...
    async leaveLobby(lobbyId: string, userId: string): Promise<void> {
        ...
        if (Object.keys(lobby.players).length > 1) {
            if (lobbyPlayer.isHost) {
                const otherPlayerId = Object.values(lobby.players).find(
                ...
                if (otherPlayerId) {
                    await this.redisService.setJson(
                        key,
                        LOBBYPATH.getHostPath(otherPlayerId),
                        true,
                    );
                }
            }
    ...
    async getAllLobbies(userId: string): Promise<Lobby[]> {
        ...
        filteredLobbies = filteredLobbies.filter(
            (lobby) => lobby.state !== LOBBY_STATE_TYPE.END,
        );
    ...
    async getLobbyByCode(code: string): Promise<Lobby | null> {
        ...
        return (
            filteredLobbies.find(
                (l) => l.code === code && Object.keys(l.players).length === 1,
            ) ?? null
        );
    }
    ...
    async inviteFriend(data: InviteFriendData, currentUserId: string) {
        ...
        await this.redisService.addToSet(
            `user:${invitedUser.id}:invitations`,
            JSON.stringify(lobbyInvite),
            this.LOBBY_INVITE_TTL,
        );
    }
    ...
    async getInvitations(currentUserId: string) {
        const invitations = await this.redisService.getSetMembers(
            `user:${currentUserId}:invitations`,
        );
        ...
        for (const invitation of invitations) {
            ...
            if (
                ...
                parsed.addresseeId === currentUserId
            ) {
    ...
    async getFriendsForInvite(
        currentUserId: string,
    ): Promise<FriendForInvite[]> {
        ...
        for (const friend of friends) {
            ...
            if (
                invitations.find(
                    (invitation) => invitation.requesterId === currentUserId,
                )
            ) {
    ...
    async joinLobbyByInvitation(invitationId: string, currentUserId: string) {
        ...
        const parsedInvitations: LobbyInvitation[] = invitations.map(
            (invitation) => JSON.parse(invitation),
        );
        const invitation = parsedInvitations.find(
        ...
        if (
            !lobby ||
            Object.keys(lobby.players).length !== 1 ||
            invitation.addresseeId !== currentUserId
        ) {
