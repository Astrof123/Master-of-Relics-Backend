...
} from 'src/lobby/types/lobby-evens-data';
...
export class LobbyGateway implements OnGatewayDisconnect {
    ...
    async handleJoinHall(
        ...
        try {
            ...
            const currentLobby =
                await this.lobbyService.getLobbyByUserId(userId);
    ...
    async handleUpdateOptionsLobby(
        client: Socket,
        data: UpdateOptionsLobbyData,
        callback: Function,
    ): Promise<void> {
        try {
            const userId = client.data.userId;
            const lobbyId = await this.lobbyService.updateOptionsLobby(
                data,
                userId,
            );
    ...
    async handleDeleteLobby(
        ...
        try {
            ...
            this.server
                .to(`lobby-${lobbyId}`)
                .emit(LOBBY_EVENT_NAME.LOBBY_UPDATE, null);
    ...
    async handleInviteFriend(
        ...
        try {
            ...
            const invitations = await this.lobbyService.getInvitations(
                data.friendId,
            );
            this.server
                .to(`user:${data.friendId}`)
                .emit(LOBBY_EVENT_NAME.YOU_INVITED, invitations);
    ...
    async handleGetFriendsForInvite(
        client: Socket,
        data: null,
        callback: Function,
    ): Promise<void> {
        try {
            ...
            const friendsForInvite =
                await this.lobbyService.getFriendsForInvite(userId);
            ...
            if (callback) {
                callback({
                    ...
                    message:
                        'Вы успешно получили список друзей для приглашения',
                });
            }
        } catch (error) {
            handleError(error, callback);
    ...
    async handleDeclineInvitation(
        client: Socket,
        lobbyInvitation: LobbyInvitation,
        callback: Function,
    ): Promise<void> {
        try {
            const userId = client.data.userId;
            ...
            const invitations = await this.lobbyService.getInvitations(userId);
            this.server
                .to(`user:${lobbyInvitation.addresseeId}`)
                .emit(LOBBY_EVENT_NAME.YOU_INVITED, invitations);
    ...
    async handleJoinLobbyByInvitation(
        client: Socket,
        invitationId: string,
        callback: Function,
    ): Promise<void> {
        try {
            ...
            const invitation = await this.lobbyService.joinLobbyByInvitation(
                invitationId,
                userId,
            );
            ...
            const currentLobby = await this.lobbyService.getLobbyById(
                invitation.lobbyId,
            );
            this.server
                .to(`lobby-${invitation.lobbyId}`)
                .emit(LOBBY_EVENT_NAME.LOBBY_UPDATE, currentLobby);
