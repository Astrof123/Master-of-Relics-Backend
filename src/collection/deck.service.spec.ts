...
const mockArtifacts: Record<string, any> = {
    ...
    'bonelord': { id: 'bonelord', hp: 35, type: 'attack', skills: null, price: 180, isForSale: true },
    'huntmaster': { id: 'huntmaster', hp: 38, type: 'attack', skills: ['hunt'], price: 190, isForSale: true },
    'grappler': { id: 'grappler', hp: 36, type: 'attack', skills: ['grab'], price: 155, isForSale: true },
    'illusion_blade': { id: 'illusion_blade', hp: 32, type: 'attack', skills: ['illusion'], price: 160, isForSale: true },
    'purifier': { id: 'purifier', hp: 35, type: 'magic', skills: ['purify'], price: 170, isForSale: true },
    'veilstrike': { id: 'veilstrike', hp: 30, type: 'attack', skills: ['veil'], price: 140, isForSale: true },
    'paladins_glove': { id: 'paladins_glove', hp: 28, type: 'support', skills: ['bless'], price: 110, isForSale: true },
    'averter': { id: 'averter', hp: 33, type: 'defense', skills: ['block'], price: 130, isForSale: true },
    'volt': { id: 'volt', hp: 27, type: 'magic', skills: ['shock'], price: 105, isForSale: true },
    'voider': { id: 'voider', hp: 27, type: 'magic', skills: ['void'], price: 105, isForSale: true },
    'glimpse': { id: 'glimpse', hp: 22, type: 'magic', skills: ['foresight'], price: 90, isForSale: true },
    'temper_crown': { id: 'temper_crown', hp: 28, type: 'support', skills: ['buff'], price: 120, isForSale: true },
    'chronos': { id: 'chronos', hp: 35, type: 'magic', skills: ['time'], price: 200, isForSale: true },
    'ring_of_destruction': { id: 'ring_of_destruction', hp: 30, type: 'attack', skills: ['destroy'], price: 125, isForSale: true },
    'concealer': { id: 'concealer', hp: 26, type: 'magic', skills: ['conceal'], price: 95, isForSale: true },
    'plunder': { id: 'plunder', hp: 34, type: 'attack', skills: ['steal'], price: 145, isForSale: true },
    'swift_boots': { id: 'swift_boots', hp: 24, type: 'support', skills: ['haste'], price: 85, isForSale: true },
    'divine_staff': { id: 'divine_staff', hp: 30, type: 'magic', skills: ['heal'], price: 175, isForSale: true },
};
...
describe('DeckService', () => {
    ...
    describe('createForNewUser', () => {
        ...
        it('should create 3 decks with default cards for new user', async () => {
            ...
            const mockDeck2 = { id: 2, userId, isActive: false, indexNumber: 2 };
            const mockDeck3 = { id: 3, userId, isActive: false, indexNumber: 3 };
    ...
    describe('getUserDecks', () => {
        ...
        it('should return user decks with cards', async () => {
            ...
            const mockAllCards = [
                { id: 10, innerCardId: 'arcane_shield', isForSale: true, price: 100 },
                { id: 11, innerCardId: 'moon_staff', isForSale: true, price: 150 },
            ];
            const mockDeckCards = [
            ...
            mockDeckCardRepository.find
                .mockResolvedValueOnce([mockDeckCards[0], mockDeckCards[1]] as DeckCard[]) 
        ...
        it('should throw UserNotFoundException if user not found', async () => {
            ...
            await expect(service.getUserDecks(userId)).rejects.toThrow(UserNotFoundException);
        });

        it('should throw CardNotFoundException if card entity not found', async () => {
            const mockUser = { id: userId, nickname: 'TestUser' };
            const mockDecks = [{ id: 1, userId, indexNumber: 1, isActive: true }];
            const mockAllCards = [{ id: 10, innerCardId: 'arcane_shield', isForSale: true, price: 100 }];
            const mockDeckCards = [{ deckId: 1, cardId: 99, position: 1 }];

            mockUsersService.findOne.mockResolvedValue(mockUser as User);
            mockDeckRepository.find.mockResolvedValue(mockDecks as Deck[]);
            mockCardRepository.find.mockResolvedValue(mockAllCards as Card[]);
            mockDeckCardRepository.find.mockResolvedValue(mockDeckCards as DeckCard[]);

            await expect(service.getUserDecks(userId)).rejects.toThrow(CardNotFoundException);
        });
    });
    ...
    describe('changeActiveDeck', () => {
        ...
        it('should throw DeckNotFoundException if new active deck not found', async () => {
            ...
            await expect(service.changeActiveDeck(userId, changeActiveDeckDto)).rejects.toThrow(DeckNotFoundException);
    ...
    describe('changeDeckCards', () => {
        ...
        it('should change deck cards successfully', async () => {
            ...
            for (let i = 1; i <= MAX_COUNT_DECK_CARDS; i++) {
                mockDeckCards.push({ id: i, deckId, cardId: i, position: i } as DeckCard);
            }
            ...
            expect(mockDeckCardRepository.save).toHaveBeenCalledTimes(MAX_COUNT_DECK_CARDS);
        });

        it('should throw InvalidNewDeckException if cards length is incorrect', async () => {
            const invalidDto = { ...changeDeckCardsDto, cards: [{ cardId: 1, position: 1 }] };

            await expect(service.changeDeckCards(userId, invalidDto)).rejects.toThrow(InvalidNewDeckException);
        ...
        it('should throw DeckNotFoundException if deck not found', async () => {
            ...
            await expect(service.changeDeckCards(userId, changeDeckCardsDto)).rejects.toThrow(DeckNotFoundException);
            expect(mockDeckCardRepository.save).not.toHaveBeenCalled();
        });
    });
    ...
    describe('createDefaultDeckCards', () => {
        ...
        it('should skip cards not found', async () => {
            ...
            mockCardRepository.findOne
                .mockResolvedValueOnce({ id: 1, innerCardId: 'arcane_shield' } as Card)
