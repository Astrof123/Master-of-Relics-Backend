...
describe('SocketConnectionService', () => {
  ...
  describe('setPlayerOnline', () => {
    ...
    it('should handle redis errors', async () => {
      ...
      await expect(service.setPlayerOnline(userId)).rejects.toThrow(error);
    });
  });
  ...
  describe('setPlayerOffline', () => {
    ...
    it('should handle redis errors', async () => {
      ...
      await expect(service.setPlayerOffline(userId)).rejects.toThrow(error);
    });
  });
  ...
  describe('getOnlinePlayers', () => {
    it('should return list of online players', async () => {
      ...
      expect(redisService.getSetMembers).toHaveBeenCalledWith('online:index');
  ...
  describe('Integration scenarios', () => {
    it('should correctly track player online/offline sequence', async () => {
      ...
      // Player goes online
      await service.setPlayerOnline(userId);
      expect(redisService.addToSet).toHaveBeenCalledWith('online:index', userId, expect.any(Number));
      ...
      // Player goes offline
      await service.setPlayerOffline(userId);
      expect(redisService.removeFromSet).toHaveBeenCalledWith('online:index', userId);
    ...
    it('should handle multiple players online/offline', async () => {
      ...
      expect(await service.getOnlinePlayers()).toEqual([user1, user2, user3]);
  ...
  describe('Edge cases', () => {
    it('should handle empty userId', async () => {
      ...
      expect(redisService.addToSet).toHaveBeenCalledWith('online:index', '', expect.any(Number));
    });
    ...
    it('should handle very long userId', async () => {
      ...
      expect(redisService.addToSet).toHaveBeenCalledWith('online:index', longUserId, expect.any(Number));
    });
    ...
    it('should handle special characters in userId', async () => {
      ...
      expect(redisService.addToSet).toHaveBeenCalledWith('online:index', specialUserId, expect.any(Number));
    });
  });
