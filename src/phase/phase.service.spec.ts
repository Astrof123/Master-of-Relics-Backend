...
import {
    ...
    LOSER_PRIZE,
    WINNER_PRIZE,
} from '../game-mechanics/constants/settings';
...
describe('PhaseService', () => {
    ...
    describe('newRound', () => {
        ...
        it('should start a new round successfully', async () => {
            ...
            expect(resourceService.addResourceNewRound).toHaveBeenCalledWith(
                mockGameState,
            );
            expect(
                gameEffectsService.checkNewRoundEffects,
            ).toHaveBeenCalledWith(mockGameState);
            expect(
                artifactStateService.updateStateNewRound,
            ).toHaveBeenCalledWith(mockGameState);
            expect(diceService.updateDicesNewRound).toHaveBeenCalledWith(
                mockGameState,
            );
        ...
        it('should reset spell cooldowns for both players', async () => {
            ...
            expect(mockGameState.player.spells.light.spell1.cooldown).toBe(
                false,
            );
            expect(mockGameState.player.spells.light.spell2.cooldown).toBe(
                false,
            );
            expect(mockGameState.player.spells.dark.spell1.cooldown).toBe(
                false,
            );
            expect(
                mockGameState.player.spells.destruction.spell1.cooldown,
            ).toBe(false);

            expect(mockGameState.enemy.spells.light.spell1.cooldown).toBe(
                false,
            );
            expect(mockGameState.enemy.spells.dark.spell1.cooldown).toBe(false);
            expect(mockGameState.enemy.spells.destruction.spell1.cooldown).toBe(
                false,
            );
        });
        ...
        it('should set current turn to player when player goes first', async () => {
            ...
            expect(
                artifactService.calculateAvailableActions,
            ).toHaveBeenCalled();
        });
    ...
    describe('calculateNewState', () => {
        ...
        it('should calculate new state for player', async () => {
            ...
            expect(
                artifactService.calculateAvailableActions,
            ).toHaveBeenCalledWith(mockGameState, player, mockGameState.enemy);
            ...
            expect(
                gameEffectsService.calculateNewStateEffects,
            ).toHaveBeenCalledWith(mockGameState, player, mockGameState.enemy);
        });
        ...
        it('should calculate new state for enemy', async () => {
            ...
            expect(
                artifactService.calculateAvailableActions,
            ).toHaveBeenCalledWith(mockGameState, enemy, mockGameState.player);
        });
