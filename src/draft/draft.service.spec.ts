...
jest.mock('../artifact/constants/artifacts', () => ({
  ARTIFACTS: {
    ...
    'axe_of_the_berserker': { id: 'axe_of_the_berserker', hp: 40, skills: ['rage'], price: 200, isForSale: true },
  },
}));
...
const createMockGameState = (userId: string, enemyId: string = 'enemy-1'): any => ({
  id: 'game-123',
  phase: PHASE.DRAFT,
  end: null,
  player: {
...
describe('DraftService', () => {
  ...
  beforeEach(async () => {
    ...
    mockArtifactService.createArtifactState.mockImplementation((artifacts, artifactId) => ({
      id: 'artifact-uuid',
      artifactId,
      state: 'ready_to_use',
      currentHp: 30,
  ...
  describe('pickArtifact', () => {
    ...
    it('should pick artifact successfully', async () => {
      ...
      expect(mockGameStateService.getGameForLogicById).toHaveBeenCalledWith('game-123', userId);
    ...
    it('should throw GAME_NOT_FOUND if game not found', async () => {
      ...
      await expect(service.pickArtifact(pickArtifactData, userId)).rejects.toThrow(GameException);
    });
    ...
    it('should throw PHASE_NOT_DRAFT if not in draft phase', async () => {
      ...
      await expect(service.pickArtifact(pickArtifactData, userId)).rejects.toThrow(DraftException);
    });

    it('should throw GAME_OVER if game ended', async () => {
      const mockGameState = createMockGameState(userId);
      mockGameState.end = { winner: 'user-123', winner_prize: 100, loser_prize: 50, draw_prize: 75 };
      mockGameStateService.getGameForLogicById.mockResolvedValue(mockGameState);
    ...
    it('should throw ARTIFACT_NOT_FOUND if artifact not in deck', async () => {
      const mockGameState = createMockGameState(userId);
      mockGameStateService.getGameForLogicById.mockResolvedValue(mockGameState);
      const invalidData = { ...pickArtifactData, artifactId: 'non_existent' };

      await expect(service.pickArtifact(invalidData, userId)).rejects.toThrow(DraftException);
    });
  });
  ...
  describe('toggleReadyDraft', () => {
    ...
    it('should toggle ready draft successfully', async () => {
      ...
      expect(mockRedisService.watch).toHaveBeenCalledWith('game:game-123');
      expect(mockGameStateService.saveGameForLogicInTransaction).toHaveBeenCalled();
    });

    it('should throw GAME_NOT_FOUND if game not found', async () => {
      mockGameStateService.getGameForLogicById.mockResolvedValue(null);

      await expect(service.toggleReadyDraft(gameId, userId)).rejects.toThrow(GameException);
    });
    ...
    it('should throw PHASE_NOT_DRAFT if not in draft phase', async () => {
      ...
      await expect(service.toggleReadyDraft(gameId, userId)).rejects.toThrow(DraftException);
    });
    ...
    it('should retry on transaction failure', async () => {
      ...
      expect(mockGameStateService.saveGameForLogicInTransaction).toHaveBeenCalledTimes(2);
    });
    ...
    it('should finish draft and transition to battle when both players ready', async () => {
      ...
      expect(artifactService.createArtifactState).toHaveBeenCalledTimes(2);
    });
    ...
    it('should start timer when draft finished', async () => {
      ...
      expect(mockGameTimerService.startTimer).toHaveBeenCalledWith(gameId, TIMER_TYPE.DRAFT, 30);
    });
  });
  ...
  describe('autoFinishDraft', () => {
    ...
    it('should return early if not in draft phase', async () => {
      ...
      expect(mockGameStateService.saveGameForLogicInTransaction).not.toHaveBeenCalled();
    });
  ...
  describe('applyFinishDraftIfNeeded', () => {
    ...
    it('should finish draft when both players are ready and have picked artifacts', async () => {
      ...
      const result = await (service as any).applyFinishDraftIfNeeded(mockGameState);
