    ...
    EXTRA_ACTIONS: {
        ...
        extra_move: {
            ...
            getDescription: jest.fn(
                () => 'Получить дополнительный ход за 15 ловкости',
            ),
            restrictions: [],
        },
        return_to_battle: {
...
describe('ExtraActionService', () => {
    ...
    describe('getExtraActions', () => {
        ...
        it('should filter actions when restrictions fail', () => {
            mockRestrictionService.checkGeneralRestrictions.mockReturnValue(
                false,
            );
            const result = service.getExtraActions(player, enemy, artifact);
        ...
        it('should filter actions when not enough resources', () => {
            ...
            expect(result.length).toBeLessThan(
                Object.keys(EXTRA_ACTIONS).length,
            );
        });
    });
    ...
    describe('handleExtraMove', () => {
        ...
        it('should add extra move', () => {
            ...
            expect(resourceService.extraMove).toHaveBeenCalledWith(
                gameState.player,
                logParts,
            );
        });
