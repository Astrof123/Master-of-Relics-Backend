...
import { Player, ArtifactGameState, ARTIFACT_STATE, ArtifactAvailableActions, SpellGameState } from '../game-state/types/game';
...
jest.mock('../game-mechanics/constants/faces', () => ({
  FACES: {
    'sword': { sword: 10, target: 0, heal: 0, description: 'Melee attack', agility: 0, rage: 0, light_mana: 0, dark_mana: 0, destruction_mana: 0 },
    'heal': { sword: 0, target: 0, heal: 10, description: 'Heal', agility: 0, rage: 0, light_mana: 0, dark_mana: 0, destruction_mana: 0 },
  },
}));
...
describe('ActionResolverService', () => {
  ...
  const createMockPlayer = (id: string = 'player-1', name: string = 'Player1'): Player => ({
    id,
    name,
    connection: 'online',
    isBot: false,
  ...
  describe('giveUpResolve', () => {
    ...
    it('should resolve give up', async () => {
      ...
      expect(phaseService.setEndGame).toHaveBeenCalledWith(gameState, gameState.enemy.id);
      expect(gameTimerService.stopAllTimers).toHaveBeenCalledWith(gameState.id);
    });
  });
  ...
  describe('autoGiveUpResolve', () => {
    ...
    it('should resolve auto give up', async () => {
      ...
      expect(gameState.logs[0].text).toContain('проиграл из-за пропуска ходов');
      expect(phaseService.setEndGame).toHaveBeenCalledWith(gameState, gameState.enemy.id);
      expect(gameTimerService.stopAllTimers).toHaveBeenCalledWith(gameState.id);
    });
  });
  ...
  describe('offerDrawResolve', () => {
    ...
    it('should end game if enemy also offered draw', async () => {
      ...
      expect(phaseService.setEndGame).toHaveBeenCalledWith(gameState, null);
    });
  });
  ...
  describe('extraActionResolve', () => {
    ...
    it('should resolve extra action', () => {
      ...
      expect(mockExtraActionService.getHandler).toHaveBeenCalledWith(EXTRA_ACTION.THROW_DICE);
  ...
  describe('toggleReadyMovementResolve', () => {
    ...
    it('should toggle ready movement', async () => {
      const result = await service.toggleReadyMovementResolve(gameState, data);

      expect(gameState.player.temporaryArtifacts).toBe(data.artifactsWithNewPosition);
  ...
  describe('autoToggleReadyMovementResolve', () => {
    ...
    it('should auto toggle ready movement', async () => {
      const result = await service.autoToggleReadyMovementResolve(gameState);
