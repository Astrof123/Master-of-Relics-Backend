...
describe('ActionGateway', () => {
  ...
  describe('handleUseFace', () => {
    ...
    it('should handle use face successfully', async () => {
      const animations: AnimationData[] = [
        { playerId: 'enemy', artifactGameId: 'enemy-artifact', animation: 'hit', value: 15 },
      ];
      ...
      expect(actionService.useFace).toHaveBeenCalledWith(useFaceData, userId, expect.any(Array));
      ...
      expect(mockServer.emit).toHaveBeenCalledWith(ACTION_EVENT_NAME.ANIMATION, animations[0]);
  ...
  describe('handleSkill', () => {
    ...
    it('should handle use skill successfully', async () => {
      const animations: AnimationData[] = [
        { playerId: 'enemy', artifactGameId: 'enemy-artifact', animation: 'hit', value: 10 },
      ];
      ...
      expect(actionService.useSkill).toHaveBeenCalledWith(useSkillData, userId, expect.any(Array));
      expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
      expect(mockServer.emit).toHaveBeenCalledWith(GAME_EVENT_NAME.GAME_STATE_UPDATED, gameId);
      expect(mockServer.emit).toHaveBeenCalledWith(ACTION_EVENT_NAME.ANIMATION, animations[0]);
      expect(mockCallback).toHaveBeenCalledWith({
  ...
  describe('handleSpell', () => {
    ...
    it('should handle use spell successfully', async () => {
      const animations: AnimationData[] = [
        { playerId: 'player', artifactGameId: 'ally-artifact', animation: 'heal', value: 20 },
      ];
      ...
      expect(actionService.useSpell).toHaveBeenCalledWith(useSpellData, userId, expect.any(Array));
      expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
      expect(mockServer.emit).toHaveBeenCalledWith(GAME_EVENT_NAME.GAME_STATE_UPDATED, gameId);
      expect(mockServer.emit).toHaveBeenCalledWith(ACTION_EVENT_NAME.ANIMATION, animations[0]);
      expect(mockCallback).toHaveBeenCalledWith({
  ...
  describe('handleEndTurn', () => {
    ...
    it('should handle end turn successfully', async () => {
      ...
      expect(actionService.endTurn).toHaveBeenCalledWith(gameId, userId, expect.any(Array));
      expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
      expect(mockServer.emit).toHaveBeenCalledWith(GAME_EVENT_NAME.GAME_STATE_UPDATED, gameId);
      expect(mockCallback).toHaveBeenCalledWith({
        success: true,
  ...
  describe('handleExtraAction', () => {
    ...
    it('should handle extra action successfully', async () => {
      ...
      expect(actionService.extraAction).toHaveBeenCalledWith(extraActionData, userId, expect.any(Array));
      expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
      expect(mockServer.emit).toHaveBeenCalledWith(GAME_EVENT_NAME.GAME_STATE_UPDATED, gameId);
      expect(mockCallback).toHaveBeenCalledWith({
        success: true,
  ...
  describe('handleToggleReadyMovement', () => {
    ...
    it('should handle toggle ready movement successfully', async () => {
      ...
      await gateway.handleToggleReadyMovement(mockSocket, toggleReadyData, mockCallback);

      expect(actionService.toggleReadyMovement).toHaveBeenCalledWith(toggleReadyData, userId);
      expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
      expect(mockServer.emit).toHaveBeenCalledWith(GAME_EVENT_NAME.GAME_STATE_UPDATED, gameId);
      expect(mockCallback).toHaveBeenCalledWith({
        success: true,
