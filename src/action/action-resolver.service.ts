...
export class ActionResolverService {
    ...
    useFaceResolve(
        ...
        const logParts: string[] = [
            `${player.name} использовал грань артефакта "${artifactName}"`,
        ];
        ...
        if (faceData.agility !== 0) {
            ...
            restoredResources.push(
                LogHelper.getRestoreAgilityLog(faceData.agility),
            );
        }
        if (faceData.rage !== 0) {
            this.resourceService.addResource(
        ...
        if (faceData.light_mana !== 0) {
            ...
            restoredResources.push(
                LogHelper.getRestoreLightManaLog(faceData.light_mana),
            );
        }
        if (faceData.dark_mana !== 0) {
            ...
            restoredResources.push(
                LogHelper.getRestoreDarkManaLog(faceData.dark_mana),
            );
        }
        if (faceData.destruction_mana !== 0) {
            this.resourceService.addResource(
                ...
                faceData.destruction_mana,
                [],
            );
            restoredResources.push(
                LogHelper.getRestoreDestructionManaLog(
                    faceData.destruction_mana,
                ),
            );
        }
        ...
        if (
            face.attackTargets !== null &&
            face.attackTargets.length > 0 &&
            data.attackTarget !== undefined
        ) {
            const damageType =
                faceData.sword > 0 ? DAMAGE.MELEE : DAMAGE.RANGED;
        ...
        if (
            face.healTargets !== null &&
            face.healTargets.length > 0 &&
            data.healTarget !== undefined
        ) {
            const healedArtifact = player.artifacts[data.healTarget!];
            const heal = this.combatService.calculateFaceHeal(
                healedArtifact,
                usedArtifact,
            );
        ...
        gameState.constants.countActionsFromStartGame += 1;
        player.movePoints -= 1;
    ...
    async endTurnResolve(gameState: GameForLogic, player: Player) {
        const enemy =
            gameState.enemy.id === player.id
                ? gameState.player
                : gameState.enemy;
        player.extraData.countActionsSinceStartTurn = 0;
        gameState.constants.countActionsFromStartGame += 1;
        ...
        if (enemy.isReady) {
            ...
            player.movePoints =
                this.resourceService.calculateNewTurnMovePoints(player);
            await this.phaseService.calculateNewState(gameState, player);
        } else {
            gameState.currentTurn = enemy.id;
            enemy.movePoints =
                this.resourceService.calculateNewTurnMovePoints(enemy);
    ...
    useSkillResolve(
        ...
        const artifactName = ARTIFACTS[artifact.artifactId].name;
        const logParts: string[] = [
            `${player.name} использовал способность артефакта "${artifactName}"`,
        ];
        ...
        gameState.logs.push(logItem);
        player.extraData.countActionsSinceStartTurn += 1;
        gameState.constants.countActionsFromStartGame += 1;
    }
    ...
    useSpellResolve(
        ...
        const logParts: string[] = [
            `${gameState.player.name} использовал заклинание "${spell.name}"`,
        ];
        strategy.execute(
            gameState,
            gameState.player,
        ...
        let cost =
            gameState.player.spells[SPELLS[data.spellId].type][data.spellId]
                .cost;
        const effect = this.gameEffectsService.getHeroEffect(
            gameState.player,
            EFFECT.FREE_SPELL,
        );
        if (effect) {
    ...
    extraActionResolve(
        ...
        const logParts: string[] = [
            `${gameState.player.name} использовал "${EXTRA_ACTIONS[data.type].name}"`,
        ];
    ...
    async toggleReadyMovementResolve(
        ...
        const bothReady =
            (gameState.enemy.isReady || gameState.enemy.isBot) &&
        ...
        if (bothReady) {
            ...
            if (gameState.enemy.isBot) {
                gameState.enemy.artifacts =
                    this.artifactService.generateNewTemporaryForBot(
                        gameState.enemy,
                    );
            } else {
            ...
            if (gameState.currentTurn === gameState.player.id) {
                await this.phaseService.calculateNewState(
                    gameState,
                    gameState.player,
                );
            } else {
