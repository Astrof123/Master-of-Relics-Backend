...
import { Player, ArtifactGameState, ARTIFACT_STATE, LINE, ArtifactAvailableActions, SpellGameState } from '../game-state/types/game';
...
jest.mock('./constants/extra-actions', () => ({
  EXTRA_ACTIONS: {
    'throw_dice': { cost: 5, resourceType: 'agility', restrictions: ['only_ready'], getDescription: jest.fn() },
    'move': { cost: 15, resourceType: 'agility', restrictions: ['only_ready'], getDescription: jest.fn() },
    'return_to_battle': { cost: 30, resourceType: 'agility', restrictions: ['only_cooldown'], getDescription: jest.fn() },
  },
}));
...
describe('ActionValidatorService', () => {
  ...
  const createMockAvailableActions = (): ArtifactAvailableActions => ({
    face: createMockFace(),
    skills: [{ id: SKILL.FEAR, description: 'Fear', possibleTargets: [[], []], countAnyTarget: 0, countTargetEnemy: 1, countTargetAllies: 0 }],
  ...
  describe('useFaceValidator', () => {
    ...
    it('should throw if game ended', () => {
      gameState.end = { winner: 'player-1', winner_prize: 100, loser_prize: 50, draw_prize: 75 };
  ...
  describe('useSkillValidator', () => {
    ...
    it('should throw if targets length is invalid', () => {
      data.targets = [[]];
      expect(() => service.useSkillValidator(gameState, artifact, data)).toThrow();
    });

    it('should throw if not enough resources', () => {
      gameState.player.resources[RESOURCE.RAGE] = 5;
      expect(() => service.useSkillValidator(gameState, artifact, data)).toThrow(ActionException);
    });
 ...
 describe('useSpellValidator', () => {
  ...
  it('should throw if spell not found', () => {
    ...
    expect(() => service.useSpellValidator(gameState, data)).toThrow(ActionException);
  });
  ...
  describe('extraActionValidator', () => {
    ...
    beforeEach(() => {
      ...
      // Настраиваем restrictionService для успешной проверки
