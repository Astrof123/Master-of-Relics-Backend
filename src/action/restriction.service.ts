...
export class RestrictionService {
    ...
    checkArtifactRestrictions(
        ...
        if (restrictions.includes(RESTRICTION.SAME_LINE_HAVE_ONE_FREE_SPOT)) {
            if (
                MAX_COUNT_ARTIFACTS_ON_LINE -
                    Object.values(player.artifacts).filter(
                        (art) => art.line === artifact.line,
                    ).length <
                1
            ) {
                return false;
        ...
        if (restrictions.includes(RESTRICTION.ZERO_USED_SKILL_CHARGES)) {
            const effect = artifact.effects.find(
                (effect) => effect.id === EFFECT.USED_SKILL_CHARGES,
            );
    ...
    checkSpellRestrictions(
        enemy: Player,
        restrictions: Restriction[],
    ): boolean {
        const artWithoutAvatar = Object.values(enemy.artifacts).find(
            (art) =>
                this.gameEffectsService.countEffect(art, EFFECT.AVATAR) === 0,
        );

        if (
            !Object.values(enemy.artifacts).find((art) => art.currentHp > 0) &&
            !artWithoutAvatar
        ) {
            return false;
        }
    ...
    checkGeneralRestrictions(
        ...
        if (restrictions.includes(RESTRICTION.ENEMY_BACK_LINE_IS_FREE)) {
            if (
                Object.values(enemy.artifacts).filter(
                    (art) => art.line === LINE.BACK,
                ).length >= MAX_COUNT_ARTIFACTS_ON_LINE
            ) {
                return false;
            }
        }
        ...
        if (restrictions.includes(RESTRICTION.HAVE_ENEMY_FOR_SKILLS)) {
            const artWithoutInvisible = Object.values(enemy.artifacts).find(
                (art) =>
                    this.gameEffectsService.countEffect(
                        art,
                        EFFECT.INVISIBLE,
            ...
            const artWithoutAvatar = Object.values(enemy.artifacts).find(
                (art) =>
                    this.gameEffectsService.countEffect(art, EFFECT.AVATAR) ===
                    0,
            );

            if (
                !Object.values(enemy.artifacts).find(
                    (art) => art.currentHp > 0,
                ) &&
                !artWithoutInvisible &&
                !artWithoutAvatar
        ...
        if (restrictions.includes(RESTRICTION.HAVE_ONE_MOVE_POINT)) {
            if (player.movePoints < 1) {
                return false;
            }
        }
        ...
        if (
            restrictions.includes(RESTRICTION.ENEMY_HAVE_TEN_DESTRUCTION_MANA)
        ) {
    ...
    getTargetsByRestrictions(
        ...
        if (restrictions.includes(TARGET_RESTRICTION.MELEE_ENEMY)) {
            ...
            Object.values(enemy.artifacts).forEach((artifact) => {
                if (
                    ...
                    artifact.state !== ARTIFACT_STATE.DESTROYED
                ) {
                    haveDefenders = true;
                    enemies.push(artifact.id);
                }
        ...
        if (restrictions.includes(TARGET_RESTRICTION.CAN_ATTACK)) {
            enemies = enemies.filter(
                (e) =>
                    FACES[enemy.artifacts[e].face].sword > 0 ||
                    FACES[enemy.artifacts[e].face].target > 0,
            );
            allies = allies.filter(
                (e) =>
                    FACES[player.artifacts[e].face].sword > 0 ||
                    FACES[player.artifacts[e].face].target > 0,
            );
        }
        if (restrictions.includes(TARGET_RESTRICTION.ALIVE)) {
            enemies = enemies.filter(
                (e) => enemy.artifacts[e].state !== ARTIFACT_STATE.BREAKEN,
            );
            allies = allies.filter(
                (e) => player.artifacts[e].state !== ARTIFACT_STATE.BREAKEN,
            );
        }
        ...
        if (restrictions.includes(TARGET_RESTRICTION.NEED_HEAL_ALLY)) {
            allies = allies.filter(
                (e) =>
                    player.artifacts[e].currentHp < player.artifacts[e].maxHp,
            );
        }
        ...
        if (restrictions.includes(TARGET_RESTRICTION.NORMAL_STATE)) {
            enemies = enemies.filter(
                (e) =>
                    enemy.artifacts[e].state === ARTIFACT_STATE.READY_TO_USE ||
                    enemy.artifacts[e].state === ARTIFACT_STATE.COOLDOWN,
            );
            allies = allies.filter(
                (e) =>
                    player.artifacts[e].state === ARTIFACT_STATE.READY_TO_USE ||
                    player.artifacts[e].state === ARTIFACT_STATE.COOLDOWN,
            );
        }
