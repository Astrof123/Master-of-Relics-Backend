...
export const EXTRA_ACTIONS: Record<ExtraAction, ExtraActionDataType> = {
    [EXTRA_ACTION.THROW_DICE]: {
        ...
        getDescription: (cost: number) =>
            `Перекинуть кубик за ${cost} ловкости`,
    ...
    [EXTRA_ACTION.EXTRA_MOVE]: {
        ...
        resourceType: RESOURCE.AGILITY,
        getDescription: (cost: number) =>
            `Получить дополнительный ход за ${cost} ловкости`,
        restrictions: [],
    },
    [EXTRA_ACTION.RETURN_TO_BATTLE]: {
        ...
        resourceType: RESOURCE.AGILITY,
        getDescription: (cost: number) =>
            `Убрать перезарядку за ${cost} ловкости`,
    ...
    [EXTRA_ACTION.REMOVE_ROOT]: {
        ...
        resourceType: RESOURCE.AGILITY,
        getDescription: (cost: number) =>
            `Снять оцепенение за ${cost} ловкости`,
        restrictions: [
    ...
    [EXTRA_ACTION.DESTROY_ARTIFACT]: {
        ...
        resourceType: RESOURCE.AGILITY,
        getDescription: () =>
            `Уничтожить артефакт, чтобы получить 30 ловкости и ярости`,
