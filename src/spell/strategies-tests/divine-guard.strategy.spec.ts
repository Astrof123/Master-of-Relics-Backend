...
jest.mock('../../game-mechanics/constants/effects', () => ({
  EFFECTS: {
    'divine_guard': { id: 'divine_guard', name: 'Divine Guard' },
  },
}));

describe('DivineGuardStrategy', () => {
  let strategy: DivineGuardStrategy;
  ...
  beforeEach(async () => {
    ...
    strategy = module.get<DivineGuardStrategy>(DivineGuardStrategy);
  ...
  describe('getSpellType', () => {
    it('should return DIVINE_GUARD', () => {
      expect(strategy.getSpellType()).toBe(SPELL.DIVINE_GUARD);
    });
  });
  ...
  describe('execute', () => {
    ...
    beforeEach(() => {
      ...
      data = {
        spellId: SPELL.DIVINE_GUARD as any,
        gameId: 'game-123',
        targets: [['artifact-1'], []],
      };
      animations = [];
    ...
    it('should apply divine guard effect to ally artifact', () => {
