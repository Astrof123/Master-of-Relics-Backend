...
export class FuryStrategy implements SpellStrategy {
    ...
    execute(gameState: GameForLogic, player: Player, data: UseSpellData, animations: AnimationData[], logParts: string[]) {
        ...
        const faceAction = this.artifactService.getFaceAction(allyArtifact, player, enemy);
        ...
        if (faceAction.attackTargets && faceAction.attackTargets.length > 0) {
            ...
            const attackedArtifact = gameState.enemy.artifacts[faceAction.attackTargets[randomEnemy]];
            ...
            this.combatService.applyDamage(gameState, enemy, allyArtifact, attackedArtifact, damage, damageType, logParts);
