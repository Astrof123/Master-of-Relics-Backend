...
describe('GameTimerService', () => {
    ...
    describe('startTimer', () => {
        ...
        it('should start timer and emit TIMER_START event', async () => {
            ...
            expect(mockGameStateService.startGameTimer).toHaveBeenCalledWith(
                gameId,
                timerType,
                duration,
            );
            expect(mockGameStateService.getTimerInfo).toHaveBeenCalledWith(
                gameId,
                timerType,
            );
            expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
            expect(mockServer.emit).toHaveBeenCalledWith(
                GAME_EVENT_NAME.TIMER_START,
                {
                    gameId,
                    timerType,
        ...
        it('should log timer start', async () => {
            ...
            expect(loggerSpy).toHaveBeenCalledWith(
                `Starting ${timerType} timer for game ${gameId}`,
            );
        });
    });
    ...
    describe('stopTimer', () => {
        ...
        it('should stop timer and emit timer:cancel event', async () => {
            ...
            expect(mockGameStateService.cancelGameTimer).toHaveBeenCalledWith(
                gameId,
                timerType,
            );
            expect(mockServer.to).toHaveBeenCalledWith(`game-${gameId}`);
        ...
        it('should log timer stop', async () => {
            ...
            expect(loggerSpy).toHaveBeenCalledWith(
                `Stopping ${timerType} timer for game ${gameId}`,
            );
        });
    });
    ...
    describe('stopAllTimers', () => {
        ...
        it('should stop all timers and emit timer:cancel-all event', async () => {
            mockGameStateService.cancelAllGameTimers.mockResolvedValue(
                undefined,
            );
            ...
            expect(
                mockGameStateService.cancelAllGameTimers,
        ...
        it('should log all timers stop', async () => {
            ...
            expect(loggerSpy).toHaveBeenCalledWith(
                `Stopping all timers for game ${gameId}`,
            );
        });
    });
    ...
    describe('syncTimersOnRecovery', () => {
        ...
        it('should sync timers on recovery when game exists', async () => {
            ...
            expect(mockGameStateService.getGameById).toHaveBeenCalledWith(
                gameId,
            );
            expect(mockGameStateService.getActiveTimer).toHaveBeenCalledWith(
                gameId,
            );
        });
    ...
    describe('Integration with server', () => {
        ...
        it('should emit to correct room for each timer type', async () => {
            ...
            for (const type of timerTypes) {
                mockGameStateService.startGameTimer.mockResolvedValue(
                    undefined,
                );
                mockGameStateService.getTimerInfo.mockResolvedValue({
                    active: true,
