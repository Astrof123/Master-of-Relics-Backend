...
describe('GameStateGateway', () => {
    ...
    describe('handleExpiredKey', () => {
        it('should handle draft timer expiration', async () => {
            ...
            mockGameStateService.getGameById.mockResolvedValue(
    ...
    describe('handleDisconnect', () => {
        ...
        it('should handle player disconnect and update connection status', async () => {
            ...
            mockGameStateService.getGameByUserId.mockResolvedValue(
                mockGame as any,
            );
            mockGameStateService.getGameForClientById.mockResolvedValue(
                mockGameForClient as any,
            );

            await gateway.handleDisconnect(mockSocket);

            expect(
                mockGameStateService.setPlayerConnectionStatus,
            ).toHaveBeenCalledWith(CONNECTIONGAME.OFFLINE, mockGame.id, userId);
    ...
    describe('handleCreateGame', () => {
        ...
        it('should create game successfully', async () => {
            ...
            mockGameStateService.createGameSessionState.mockResolvedValue(
                gameId,
            );
            ...
            expect(
                mockGameStateService.createGameSessionState,
            ).toHaveBeenCalledWith(lobbyId, userId);
            expect(mockSocket.to).toHaveBeenCalledWith(`lobby-${gameId}`);
            expect(mockSocket.emit).toHaveBeenCalledWith(
                LOBBY_EVENT_NAME.GAME_STARTED,
                gameId,
            );
        ...
        it('should handle errors', async () => {
            ...
            mockGameStateService.createGameSessionState.mockRejectedValue(
                error,
            );
    ...
    describe('handleCreateGameWithBot', () => {
        ...
        it('should create game with bot successfully', async () => {
            ...
            mockGameStateService.createGameSessionWithBotState.mockResolvedValue(
                gameId,
            );
            mockLobbyService.getLobbyById.mockResolvedValue(mockLobby as any);

            await gateway.handleCreateGameWithBot(
                mockSocket,
                lobbyId,
                mockCallback,
            );
            ...
            expect(
                mockGameStateService.createGameSessionWithBotState,
            ).toHaveBeenCalledWith(lobbyId, userId);
            expect(mockCallback).toHaveBeenCalledWith({
                success: true,
                data: { gameId },
    ...
    describe('handleJoinGame', () => {
        ...
        it('should join game successfully', async () => {
            ...
            mockGameStateService.setPlayerConnectionStatus.mockResolvedValue(
                undefined,
            );
            mockGameStateService.getActiveTimer.mockResolvedValue(
                TIMER_TYPE.TURN,
            );
            mockGameStateService.getTimerInfo.mockResolvedValue(
                mockTimerInfo as any,
            );

            await gateway.handleJoinGame(mockSocket, gameId, mockCallback);

            expect(
                mockGameStateService.getGameForClientById,
            ).toHaveBeenCalledWith(gameId, userId);
            expect(mockSocket.join).toHaveBeenCalledWith(`game-${gameId}`);
            expect(
                mockGameStateService.setPlayerConnectionStatus,
            ).toHaveBeenCalledWith(CONNECTIONGAME.ONLINE, gameId, userId);
            expect(mockCallback).toHaveBeenCalledWith({
                success: true,
                data: {
                    gameState: mockGame,
    ...
    describe('Timer handlers', () => {
        ...
        describe('handleDraftTimeout', () => {
            it('should auto finish draft for players not ready', async () => {
                ...
                expect(draftService.autoFinishDraft).toHaveBeenCalledWith(
                    gameId,
                    'user-1',
                );
                expect(draftService.autoFinishDraft).not.toHaveBeenCalledWith(
                    gameId,
                    'user-2',
                );
            });
        ...
        describe('handleMovementTimeout', () => {
            it('should auto toggle ready for players not ready', async () => {
                ...
                actionService.autoToggleReadyMovement.mockResolvedValue(
                    undefined,
                );
                ...
                expect(
                    actionService.autoToggleReadyMovement,
                ).toHaveBeenCalledWith(gameId, 'user-1');
                expect(
                    actionService.autoToggleReadyMovement,
                ).not.toHaveBeenCalledWith(gameId, 'user-2');
            });
        });
        ...
        describe('handleTurnTimeout', () => {
            it('should auto end turn for current player', async () => {
                ...
                expect(actionService.autoEndTurn).toHaveBeenCalledWith(
                    gameId,
                    'user-1',
                );
                expect(actionService.autoEndTurn).not.toHaveBeenCalledWith(
                    gameId,
                    'user-2',
                );
            });
            ...
            it('should auto give up if player skipped too many turns', async () => {
                ...
                expect(actionService.autoGiveUp).toHaveBeenCalledWith(
                    gameId,
                    'user-1',
                );
            });
